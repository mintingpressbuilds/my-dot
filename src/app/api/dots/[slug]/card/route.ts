...
export async function GET(
  ...
  try {
    ...
    const lnk = dot.link
      ...
      : '';
    const cRgba18 = hexToRgba(dot.color, 0.18);
    const cRgba05 = hexToRgba(dot.color, 0.05);
    const cRgba07 = hexToRgba(dot.color, 0.07);
    ...
    parts.push('.sh a{color:rgba(255,255,255,.25);text-decoration:none}');
    parts.push('.sh a:hover{color:rgba(255,255,255,.5)}');
    ...
    parts.push('<\/div><\/div>');
    parts.push('<div class="sh">made with <a href="https://mydot.space?ref=' + escH(slug) + '">my dot.<\/a><\/div>');
    parts.push('<\/div>');
    parts.push('<script>');
    ...
    const filename = dot.name.toLowerCase().replace(/\s+/g, '-') + '.dot.html';
